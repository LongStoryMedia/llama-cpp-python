...
Purpose: Python bindings + high-level / server wrappers around llama.cpp. By default uses externally built `llama.cpp` library for performance, minimal deps, alignment with upstream C API.

### Big Picture
- Core C/C++ library can be built via `scikit-build-core` + CMake (legacy internal build) or used from external installation (default). We expose:
...
- External llama.cpp is preferred; vendored sources in `vendor/llama.cpp` exist for legacy builds only; Python layer adapts interface and avoids adding heavy deps.
...
### Key Workflows
- External llama.cpp install (preferred): Install external llama.cpp with shared libraries, then `pip install llama-cpp-python`.
- Internal build install (legacy): `CMAKE_ARGS="-DLLAMA_BUILD=ON -DLLAMA_USE_EXTERNAL=OFF" pip install -e .` (Make target `make build.internal`). Use env var `CMAKE_ARGS` for backend flags (e.g. `-DGGML_METAL=on`).
...
- Mirror upstream C changes: when upstream `llama.h` changes, sync signatures/types in `llama_cpp/llama_cpp.py` and adjust wrapper logic in `llama.py` (search for changed enum names/constants first).
...
- Updating vendor code: perform submodule update (`git submodule update --init --recursive`), then `make clean` and rebuild; validate by importing and calling a trivial `Llama(model_path=..., vocab_only=True)`. Note: external llama.cpp is preferred over vendor code.
...
1. For external build: install llama.cpp externally, then `make build` (or specific backend). For internal build: `make build.internal` (or specific backend). 2. `pytest -q`. 3. Smoke load: `python - <<'PY'\nfrom llama_cpp import Llama; print(Llama(model_path='path/to/model.gguf', vocab_only=True))\nPY`. 4. Optional server start & single chat request.
...
### If Unsure
Prefer inspecting upstream llama.cpp for authoritative behavior (externally installed or `vendor/llama.cpp` if using internal build), then replicate minimally in Python wrappers.
